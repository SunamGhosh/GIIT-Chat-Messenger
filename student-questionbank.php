...
// Build tabs+data ahead of time so PHP isn't inside the heavy CSS block
$tabs = [];
for ($i = 1; $i <= $max; $i++) {
    $label = ($cMode == 's' || $cMode == 'S') ? "Semester $i" : "Year $i";
    $shortLabel = ($cMode == 's' || $cMode == 'S') ? "Sem $i" : "Year $i";
    $sql = "
        SELECT qb.questionbank_id, qb.questionbank_name, qb.questionbank_file, sub.subject_name
...
<head>
    ...
    <meta name="description" content="GIIT Question Bank – Download previous year question papers and practice papers.">
    <meta name="author" content="Sunam Ghosh --@SunamGhosh">
    <title>Question Bank | GIIT Professional College</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box !important; }
        ...
        @media (max-width: 480px) {
            ...
            #qbPage > .sem-selector, #qbPage > .search-bar-wrap { padding: 0 10px; }
...
<div id="qbPage">
    ...
    <div class="page-header">
        <div class="page-title-text">
            Question Bank
            <span>Browse &amp; download previous year question papers</span>
        </div>
        <div class="course-pill">
            <span class="cp-icon"><i class="fa fa-database"></i></span>
            <span>
                <?php echo htmlspecialchars($_SESSION['course']['course_name'] ?? ''); ?>
                <span style="opacity:.7;font-size:.8em;margin-left:3px;">(<?php echo htmlspecialchars($_SESSION['course']['university_short_name'] ?? ''); ?>)</span>
            </span>
        </div>
    </div>
    ...
    <div class="sem-selector" id="semSelector">
        <?php foreach ($tabs as $idx => $tab): ?>
            <button
                ...
                data-tab="tab<?= $idx ?>"
                aria-label="Show <?= htmlspecialchars($tab['label']) ?>"
                id="semBtn<?= $idx ?>">
    ...
    <div class="tab-panel <?= $idx === 0 ? 'active' : '' ?>" id="tab<?= $idx ?>">
        ...
        <?php if (empty($rows)): ?>
            <div class="empty-state">
                ...
                <p>No question papers available for this <?= ($cMode=='s'||$cMode=='S') ? 'semester' : 'year' ?> yet.<br>Check back later!</p>
            </div>
        <?php else: ?>
                ...
                <div class="qb-table-wrap">
                    <table class="qb-table">
                        ...
                        <?php foreach ($grouped as $subjectName => $papers): ?>
                            ...
                            ?>
                            <tr class="qb-row"
                                data-name="<?= htmlspecialchars(strtolower($r['questionbank_name'])) ?>"
                                data-subject="<?= htmlspecialchars(strtolower($r['subject_name'])) ?>">
                                ...
                                <td class="col-act">
                                    <a href="<?= htmlspecialchars($r['questionbank_file']) ?>"
                                       ...
                                       aria-label="Download <?= htmlspecialchars($r['questionbank_name']) ?>">
